...
2. **通用兜底**：其余所有工具自动加 `qt_` 前缀，如 TaskCreate→qt_TaskCreate
   mcp__playwright__click→qt_mcp__playwright__click 等。
   `qt_` 前缀保证名字空间不会与 Qwen 内置函数碰撞。

出站（发往 Qwen）：客户端名 → 别名 / qt_前缀名
入站（Qwen 返回）：别名 / qt_前缀名 → 客户端名
"""
...
def to_qwen_name(name: str) -> str:
    ...
    - 无显式别名：加 qt_ 前缀（如 TaskCreate → qt_TaskCreate）
    - 空值：原样返回
    - 大小写不敏感匹配别名表
    """
...
def from_qwen_name(name: str) -> str:
    ...
    - 命中 qt_ 前缀：剥掉前缀（qt_TaskCreate → TaskCreate）
